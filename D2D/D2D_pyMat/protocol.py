...
def build_protocol_frames_from_bytes(
    ...
    *,
    ...
    """Build protocol frames from an in-memory payload.

    This is used by raw-RGB payload mode while keeping the normal PMAT frame
    format.
    """
    config.validate()
    ...
    *,
    ...
    for frame in selected:
        if frame.chunk_count != first.chunk_count:
            continue
        if 0 <= frame.chunk_index < frame.chunk_count and frame.chunk_index not in chunks:
    ...
    filename = output_name or first.filename

    if complete:
        recovered_bytes = b"".join(chunks[index] for index in range(first.chunk_count))[
            : first.total_size
        ]
        crc_ok = (zlib.crc32(recovered_bytes) & 0xFFFFFFFF) == first.file_crc32
        # Default behavior only persists bytes when the file-level CRC is OK.
        # Debug callers can set require_file_crc=False to materialize a corrupt
        # reconstruction for visual inspection under low-SNR conditions.
